...
void python_to_enfa::scan_to_file(std::ostream &out, const std::string &file) const {
    ...
    for (unsigned long j = 0; j < chara.size(); j++) {
        ...
        if (chara[j].first == ' ') { to_print = "SPACE"; }
        if (chara[j].first == '\n') { to_print = "NEWLINE"; }

        out << to_print << " " << chara[j].second.first << " " << chara[j].second.second << '\n';
    }
}
        ...
        if (it->first == '#') {
            ...
            for (it2 = chars.begin(); it2 != chars.end(); it2++) {
                if (it2->second.second == row and it2->second.first < column and it2->first == '"') {
                    if (quote_single_matched) { quote_double_count++; }
                ...
                if (it2->second.second == row and it2->second.first < column and it2->first == '\'') {
                    if (quote_double_matched) { quote_single_count++; }
            ...
            if (quote_double_matched and quote_single_matched) {
                ...
                result[std::make_pair(std::make_pair(it->second.first, it->second.second),
                                      std::make_pair(it->second.first, it->second.second + str_length))] = comment;
            }
        }
        ...
        if (index < chars_size - 5) {
            if (chars[index].first == '"' and chars[index + 1].first == '"' and chars[index + 2].first == '"' and
                ...
                while (index_to_skip < chars_size - 2) {
                    if (chars[index_to_skip].first == '"' and chars[index_to_skip + 1].first == '"' and
                        chars[index_to_skip + 2].first == '"') {
                        break;
                    }
                ...
                std::pair<int, int> end_p =
                  std::make_pair(chars[index_to_skip].second.first, chars[index_to_skip].second.second);
