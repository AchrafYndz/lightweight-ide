...
DFA NFA::toDFA() const {
  ...
  State *start;
  for (auto state : states) if (state->starting) start = state;
  ...
  DFAStates.insert({stateSetToName(init), new State{ .name = stateSetToName(init), .starting = true, .accepting = start->accepting }});
  ...
  while (!queue.empty()) {
    ...
    for (const char letter : alphabet) {
      ...
      for (auto t : targets) if (t->accepting) accepting = true;
      ...
      if (DFAStates.end() == DFAStates.find(targetName)) {
        ...
        // Queue the new combined state for analysis if required
				...
				if (targetName == "∅") for (const char letter : alphabet) DFATransitions.push_back(new Transition{ .from=state, .to=state, .input=letter });
				else queue.push_back(targets);
      } else state = DFAStates.at(targetName);
